...
- **济南**、**青岛** → `…/jinan-qingdao.html`（同一公开相册页；地球上分设两个目的地）
...
| Cesium ion 令牌 | `.env` → `/api/config` | Secret `VITE_CESIUM_ION_TOKEN`（写入前端包；请用受限客户端令牌；**未配置时静默基础地球**） |
...
- **分享视角深链**：地图右上角复制链接（`#v=lon,lat,height,heading,pitch,roll`，正在查看景点时追加 `&p=目的地 id`），Pages `/3D-Earth/` 与本地均可还原相机；带 `p` 时同时打开该景点。旧链接没有 `p` 时仍只还原相机。
- **旅程**：侧栏「旅程」按 xixia-heritage 已发布线路（大西北、阿里、成都、徽州、桐江、济南青岛）飞到沿线城市。本机有高德 Key 时折线沿驾车路线；Pages 没有后端，仍用测地线。相册仍深链公开站。
- 22 个中国大陆目的地：九寨沟、故宫、张家界、西湖、黄山、八达岭、兵马俑、漓江阳朔、布达拉宫、外滩、莫高窟、丽江、贺兰山、西夏陵、宝塔山、壶口瀑布、西藏阿里、成都、婺源、桐江书院、济南、青岛。
...
5. 全景既可由“打开720全景”进入，也可点击地图上的 `720全景` 标记。落在目的地 20 公里内的全景，会出现在该景点详情里。拖动环视，滚轮缩放，方向键转动，Esc 返回。
6. 3D Tiles 以 `tileset.json` 的地理变换为准，不重复套用普通模型的经纬度变换；图层状态显示瓦片加载进度与失败片段。
7. 点击 BIM / 模型或带 feature 属性的瓦片，打开右侧属性面板；点击空白处关闭高亮。

### 分享视角（Share camera）

- 地图顶栏 **链接图标** 复制当前相机：`#v=经度,纬度,高度m,航向°,俯仰°,滚转°`。当时打开了景点时，末尾追加 `&p=景点 id`。
- 打开带该 hash 的 URL（含 `https://…/3D-Earth/#v=…`）会在地球就绪后飞到同一视角；有 `p` 时选中该景点。无后端，适合 Pages。不含 `p` 的旧链接仍然可用。
